...
export class PawnShopUtils {
  ...
  public static async closePosition(id: number, signer: SignerWithAddress, shop: TetuPawnShop): Promise<void> {
    console.log('Try to close position', id);
    await shop.connect(signer).closePosition(id);
  }

  public static async bid(id: number, amount: string, signer: SignerWithAddress, shop: TetuPawnShop) {
  ...
  public static async claim(id: number, signer: SignerWithAddress, shop: TetuPawnShop) {
    console.log('Try to claim on position', id);
    await shop.connect(signer).claim(id);
  }

  public static async redeem(id: number, signer: SignerWithAddress, shop: TetuPawnShop) {
  ...
  public static async closeAuctionBid(bidId: number, signer: SignerWithAddress, shop: TetuPawnShop) {
    console.log('Try to close auction bid', bidId);
    await shop.connect(signer).closeAuctionBid(bidId);
  }

  public static async acceptAuctionBid(posId: number, signer: SignerWithAddress, shop: TetuPawnShop) {
