...
export class PawnShopTestUtils {
  ...
  public static async closeAndCheck(
    ...
    const lastLoanListIndexAfter = (
      await shop.posIndexes(0, lastLoanId)
    ).toString();
  ...
  public static async bidAndCheck(
    loanId: number,
    amount: string,
    signer: SignerWithAddress,
    shop: TetuPawnShop
    ...
    await PawnShopTestUtils.checkExecution(
      loanId,
      amount,
      signer.address,
      shop,
  ...
  public static async checkExecution(
    ...
    if (
      lAfter.info.posDurationBlocks.isZero() &&
      !lAfter.acquired.acquiredAmount.isZero()
    ) {
