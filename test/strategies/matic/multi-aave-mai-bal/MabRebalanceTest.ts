...
export class MabRebalanceTest extends SpecificStrategyTest {
  public async do(deployInfo: DeployInfo): Promise<void> {
    it('Rebalance on matic price change', async () => {
      ...
      const { stablecoinAddress, priceSlotIndex } =
        AMBUtils.getSlotsInfo(underlying);
      ...
      const collateralPercentage0 =
        await strategyAaveMaiBal.collateralPercentage();
      ...
      const adrBytes32 =
        '0x' + '0'.repeat(24) + mockPriceSource.address.slice(2);
