...
export class IronDoHardWork extends DoHardWorkLoopBase {

  public async loopStartActions(i: number) {
    await super.loopStartActions(i);
    const start = Date.now();
    const foldContract = await DeployerUtils.connectInterface(this.signer, 'StrategyIronFold', this.strategy.address) as StrategyIronFold
