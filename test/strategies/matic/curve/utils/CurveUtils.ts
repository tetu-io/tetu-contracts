...
import { FeeRewardForwarder, IAavePool, IERC20, IRenBTCPool, RewardToken } from "../../../../../typechain";
...
async function configureFeeRewardForwarder(feeRewardForwarder: FeeRewardForwarder, rewardToken: RewardToken) {
    for (let rt of [MaticAddresses.WMATIC_TOKEN, MaticAddresses.CRV_TOKEN]) {
...
async function addLiquidityAave(investor:SignerWithAddress) {
...
async function addLiquidityRen(investor:SignerWithAddress) {
    ...
    let renBTCPool = await ethers.getContractAt("IRenBTCPool", MaticAddresses.CURVE_renBTC_POOL, investor) as IRenBTCPool;
...
async function swapTokensAAVE(trader:SignerWithAddress) {
    ...
    let usdcUserBalance = await usdcToken.balanceOf(trader.address);
    expect(usdcUserBalance).is.not.eq("0", "user should have some USDC tokens to swap");
    let depContract = await ethers.getContractAt("IAavePool", MaticAddresses.CURVE_AAVE_POOL, trader) as IAavePool;
    ...
    let daiTokenBalance = await daiToken.balanceOf(trader.address);
...
export {configureFeeRewardForwarder, addLiquidityAave, addLiquidityRen, swapTokensAAVE};