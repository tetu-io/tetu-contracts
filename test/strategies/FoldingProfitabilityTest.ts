...
export class FoldingProfitabilityTest extends SpecificStrategyTest {
  public async do(deployInfo: DeployInfo): Promise<void> {
    it("Folding profitability test", async function () {
      ...
      const lendingUnderlyingProfitUSD =
        underlyingProfitLending * underlyingUSDPrice;
      const foldingUnderlyingProfitUSD =
        underlyingProfitFolding * underlyingUSDPrice;

      const lendingTetuProfitUSD = tetuProfitLending * tetuUSDCPrice;
      const foldingTetuProfitUSD = tetuProfitFolding * tetuUSDCPrice;

      const totalLendingProfitUSD =
        lendingUnderlyingProfitUSD + lendingTetuProfitUSD;
      const totalFoldingProfitUSD =
        foldingTetuProfitUSD + foldingUnderlyingProfitUSD;
  ...
  public static async hardWork(
    ...
    const tetuEarned =
      +utils.formatUnits(await bookkeeper.targetTokenEarned(strategy.address)) -
      tetuEarnedBefore;
