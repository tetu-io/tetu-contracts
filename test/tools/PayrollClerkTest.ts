...
describe("Payroll Clerk tests", function () {
  ...
  it("should pay salary + change wallet", async () => {
    ...
    const tPrice = await calculator.getPriceWithDefaultOutput(core.rewardToken.address);
  ...
  it("should not pay salary for unknown worker", async () => {
    const tPrice = await calculator.getPriceWithDefaultOutput(core.rewardToken.address);
    await expect(clerk.pay(signer.address, 1, core.rewardToken.address, tPrice)).rejectedWith('worker not registered');
  });
  ...
  it("should not pay salary without funds", async () => {
    ...
    await clerk.changeTokens([core.rewardToken.address], [100]);
    const tPrice = await calculator.getPriceWithDefaultOutput(core.rewardToken.address);
    await expect(clerk.pay(signer.address, 1, core.rewardToken.address, tPrice)).rejectedWith('not enough fund');
  });
