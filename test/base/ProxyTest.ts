...
describe('Proxy tests', function () {
  ...
  it('upgrade proxy v 1-1-0', async () => {
    ...
    it('upgrade proxy v 1-1-0', async () => {
      ...
      await vault.notifyTargetRewardAmount(tetu, utils.parseUnits('100'));
      ...
      expect(
        +utils.formatUnits(await vault.earned(tetu, signer.address)),
      ).is.eq(0);
    ...
    it('forbidden and not a contract test', async () => {
      ...
      await expect(vaultProxy1.upgrade(vault.address)).is.rejectedWith(
        'forbidden',
      );
      await vault.initializeSmartVault(
        'TETU_PS1',
    ...
    it('wrong impl test', async () => {
      ...
      await expect(
        core.controller.upgradeTetuProxyBatch(
          [vault.address],
          [newVaultLogic.address],
        ),
