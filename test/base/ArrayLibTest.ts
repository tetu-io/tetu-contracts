...
describe('Array lib tests', function () {
  ...
  before(async function () {
    ...
    arraylib = (await DeployerUtils.deployContract(
      signer,
      'ArrayLibTest',
    )) as ArrayLibTest;
  ...
  it('contains test', async () => {
    ...
    expect(await arraylib.callStatic.containsAddress(addresses, usdc)).is.equal(
      true,
    );
    expect(await arraylib.callStatic.containsAddress(addresses, tetu)).is.equal(
      false,
    );
  });
  ...
  it('add unique test', async () => {
    ...
    await expect(
      arraylib.callStatic.addUniqueUint(uintArray, 1),
    ).is.rejectedWith(NOT_UNIQUE_ITEM);
    const addressArray = await arraylib.callStatic.addUniqueAddress(
      addresses,
      tetu,
    );
    expect(
      await arraylib.callStatic.containsAddress(addressArray, tetu),
    ).is.equal(true);
    await expect(
      arraylib.callStatic.addUniqueAddress(addressArray, usdc),
    ).is.rejectedWith(NOT_UNIQUE_ITEM);
  });

  it('add unique array test', async () => {
    const uintArray = await arraylib.callStatic.addUniqueArrayUint(
      uints,
      [5, 6],
    );
    expect(await arraylib.callStatic.containsUint(uintArray, 6)).is.equal(true);
    await expect(
      arraylib.callStatic.addUniqueArrayUint(uintArray, [1, 7]),
    ).is.rejectedWith(NOT_UNIQUE_ITEM);
    const addressArray = await arraylib.callStatic.addUniqueArrayAddress(
      addresses,
      [tetu],
    );
    expect(await arraylib.callStatic.containsUint(addressArray, tetu)).is.equal(
      true,
    );
    await expect(
      arraylib.callStatic.addUniqueArrayUint(addressArray, [usdc]),
    ).is.rejectedWith(NOT_UNIQUE_ITEM);
  });

  it('remove by index test', async () => {
    const uintArray = await arraylib.callStatic.removeByIndexUint(
      uints,
      0,
      true,
    );
    expect(await arraylib.callStatic.containsUint(uintArray, 1)).is.equal(
      false,
    );
    await expect(
      arraylib.callStatic.removeByIndexUint(uintArray, 10, true),
    ).is.rejectedWith(INDEX_OUT_OF_BOUND);
    const addressArray = await arraylib.callStatic.removeByIndexAddress(
      addresses,
      0,
      true,
    );
    expect(
      await arraylib.callStatic.containsAddress(addressArray, usdc),
    ).is.equal(false);
    await expect(
      arraylib.callStatic.removeByIndexAddress(addressArray, 10, true),
    ).is.rejectedWith(INDEX_OUT_OF_BOUND);
  });

  it('find and remove test', async () => {
    const uintArray = await arraylib.callStatic.findAndRemoveUint(
      uints,
      1,
      false,
    );
    ...
    await expect(
      arraylib.callStatic.findAndRemoveUint(uintArray, 5, false),
    ).is.rejectedWith(ITEM_NOT_FOUND);
    const addressArray = await arraylib.callStatic.findAndRemoveAddress(
      addresses,
      usdc,
      false,
    );
    ...
    await expect(
      arraylib.callStatic.findAndRemoveAddress(addressArray, tetu, false),
    ).is.rejectedWith(ITEM_NOT_FOUND);
  });

  it('find and remove array test', async () => {
    const uintArray = await arraylib.callStatic.findAndRemoveArrayUint(
      uints,
      [1, 3],
      true,
    );
    expect(await arraylib.callStatic.containsUint(uintArray, 3)).is.equal(
      false,
    );
    await expect(
      arraylib.callStatic.findAndRemoveArrayUint(uintArray, [1, 5], false),
    ).is.rejectedWith(ITEM_NOT_FOUND);
    const addressArray = await arraylib.callStatic.findAndRemoveArrayAddress(
      addresses,
      [usdc],
      true,
    );
    ...
    await expect(
      arraylib.callStatic.findAndRemoveArrayAddress(
        addressArray,
        [tetu],
        false,
      ),
  ...
  it('sort array by int reverted', async () => {
    ...
    const ints = [5, 10, 0, 9];
    const result = await arraylib.callStatic.sortAddressesByUintReverted(
      adrs,
      ints,
    );
    console.log(result);
