...
describe('Tetu Swap base tests', function () {
  ...
  before(async function () {
    ...
    vault0 = (
      await DeployerUtils.deployDefaultNoopStrategyAndVault(
        signer,
        core.controller,
    ...
    factory = (
      await DeployerUtils.deploySwapFactory(signer, core.controller.address)
    )[0] as TetuSwapFactory;
    router = (await DeployerUtils.deployContract(
      signer,
      'TetuSwapRouter',
      factory.address,
      networkToken,
  ...
  it('healthy K after vault manipulations', async () => {
    ...
    await lpCtr.sync();
    await lpCtr.swap(0, amountOut, signer.address, '0x');
  });
