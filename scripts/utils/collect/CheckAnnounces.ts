...
async function main() {
  ...
  const timeLockInfosLength = (
    await announcer.timeLockInfosLength()
  ).toNumber();
  ...
  for (let i = timeLockInfosLength - 1; i >= 0; i--) {
    ...
    const ts = (
      await announcer.timeLockSchedule(timeLockInfo.opHash)
    ).toNumber();
    if (ts !== 0) {
      console.log('ts', ts, new Date(ts * 1000), Date.now() / 1000);
      console.info('timeLockInfo', timeLockInfo.opCode, timeLockInfo);
      const txt = `${i},${new Date(ts * 1000)},${
        IDS.get(timeLockInfo.opCode) ?? timeLockInfo.opCode
      },${timeLockInfo.opHash}, ${timeLockInfo.target}, ${
        timeLockInfo.adrValues
      },${timeLockInfo.numValues}\n`;
