...
async function version(contract: string, signer: SignerWithAddress, name: string): Promise<string> {
  if (contract === '0x0000000000000000000000000000000000000000' || contract === '') {
    return 'empty';
  }
  const ctr = await DeployerUtils.connectInterface(signer, 'IVersion', contract) as IVersion;
  try {
