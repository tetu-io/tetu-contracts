...
contract PayrollClerk is PayrollClerkStorage {
    ...
    function initialize(address _controller, address _calculator)
        external
        initializer
    {
    ...
    function pay(
        ...
        require(
            salaryToken <= ERC20(token).balanceOf(address(this)),
            "not enough fund"
        );
    ...
    function computeSalary(
        ...
        salaryUsd = hRate
            .mul(_workedHours)
            .mul(1e18)
            .mul(tokenRatios[token])
            .div(FULL_RATIO);

        // return token amount with token decimals
        salaryToken = salaryUsd
            .mul(1e18)
            .div(tPrice)
            .mul(10**ERC20(token).decimals())
            .div(1e18);
    }
    ...
    function hourlyRate(address worker) public view returns (uint256) {
        ...
        if (boostActivated[worker]) {
            ratio = Math.min(
                workedHours[worker].div(BUST_STEP).add(1),
                MAX_BOOST
            );
        }
    ...
    function changeWorkerAddress(address oldWallet, address newWallet)
        external
    ...
    function setWorkerName(address worker, string calldata name)
        public
        onlyControllerOrGovernance
    {
    ...
    function setWorkerRole(address worker, string calldata role)
        public
        onlyControllerOrGovernance
    {
    ...
    function setBaseHourlyRate(address worker, uint256 rate)
        public
        onlyControllerOrGovernance
    {
    ...
    function switchBoost(address worker, bool active)
        external
        onlyControllerOrGovernance
    {
        require(baseHourlyRates[worker] != 0, "worker not registered");
    ...
    function moveTokensToGovernance(address _token, uint256 amount)
        external
        onlyControllerOrGovernance
    {
