...
import "../../../third_party/curve/ICurveDeposit3tokenUnderlying.sol";
import "../../../third_party/curve/Gauge.sol";
import "../StrategyBase.sol";


/// @title Abstract contract for Wault strategy implementation
/// @author belbix
abstract contract CurveAaveStrategyFullBuyback is StrategyBase {
  ...
  constructor(
    ...
    address lpToken = ICurveDeposit3tokenUnderlying(_pool).lp_token();
  ...
  function rewardPoolBalance() public override view returns (uint256 bal) {
    bal = Gauge(pool).balanceOf(address(this));
  }

  /// @notice Return approximately amount of reward tokens ready to claim in Aave Gauge pool
  ...
  function readyToClaim() external view override returns (uint256[] memory) {
    ...
    for (uint256 i = 0; i < _rewardTokens.length; i++) {
      toClaim[i] = Gauge(pool).claimable_reward(address(this), _rewardTokens[i]);  
    }
    return toClaim;
  }
